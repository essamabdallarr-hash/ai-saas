...
/** يُزيل الأسرار المشفرة من كائن المستأجر قبل الإرسال */
function sanitizeTenant<T extends { openaiApiKeyEnc?: string | null; metaAccessTokenEnc?: string | null }>(tenant: T): Omit<T, 'openaiApiKeyEnc' | 'metaAccessTokenEnc'> & { openaiKeyConfigured: boolean; metaConfigured: boolean } {
  const { openaiApiKeyEnc, metaAccessTokenEnc, ...rest } = tenant;
...
export async function login(req: Request, res: Response): Promise<void> {
  ...
  res.json({ token, user: publicUser(user), tenant: tenant ? sanitizeTenant(tenant) : null });
}
...
export async function me(req: Request, res: Response): Promise<void> {
  ...
  const tenant = user.tenantId
    ...
    : null;
  res.json({ user: publicUser(user), tenant: tenant ? sanitizeTenant(tenant) : null });
}
