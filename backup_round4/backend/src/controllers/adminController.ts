...
/** تحديث كامل لوكيل مستأجر — بوابة AI Studio (Super Admin فقط) */
export async function updateTenantAgent(req: Request, res: Response): Promise<void> {
  ...
  const body = req.body ?? {};
  const { dynamicFields, documents: _documents, systemPrompt, ...fields } = body;

  const updateData: Record<string, unknown> = {};
  for (const key of ['name', 'status', 'language', 'objective', 'voiceProvider', 'voiceId', 'voiceRate', 'sttProvider', 'llmProvider', 'llmModel', 'sileroVadEnabled', 'bargeInEnabled', 'smartTtsCacheEnabled', 'fallbackPhoneNumber', 'maxTurnsBeforeHandoff']) {
    if (fields[key] !== undefined) updateData[key] = fields[key];
  }
  if (typeof systemPrompt === 'string') {
    updateData.systemPrompt = systemPrompt;
    updateData.promptVersion = { increment: 1 };
  }

  const updated = await prisma.agent.update({ where: { id: agentId }, data: updateData });
...
/** قراءة نتائج مستأجر — Super Admin فقط */
export async function listTenantOutcomes(req: Request, res: Response): Promise<void> {
...
/** إنشاء نتيجة لمستأجر — Super Admin فقط */
export async function createTenantOutcome(req: Request, res: Response): Promise<void> {
  const { tenantId } = req.params;
  const tenant = await prisma.tenant.findUnique({ where: { id: tenantId } });
  if (!tenant) throw new ApiError(404, 'المستأجر غير موجود', 'TENANT_NOT_FOUND');
...
/** تعديل نتيجة لمستأجر — Super Admin فقط */
export async function updateTenantOutcome(req: Request, res: Response): Promise<void> {
  const { tenantId, outcomeId } = req.params;
  const tenant = await prisma.tenant.findUnique({ where: { id: tenantId } });
  if (!tenant) throw new ApiError(404, 'المستأجر غير موجود', 'TENANT_NOT_FOUND');
  ...
  if (Object.keys(updateData).length === 0) {
    throw new ApiError(422, 'يلزم label أو position', 'MISSING_FIELDS');
  }

  const updated = await prisma.tenantOutcome.update({ where: { id: outcomeId }, data: updateData });
...
/** حذف نتيجة لمستأجر — Super Admin فقط مع فحص سلامة البيانات */
export async function deleteTenantOutcome(req: Request, res: Response): Promise<void> {
  const { tenantId, outcomeId } = req.params;
  const tenant = await prisma.tenant.findUnique({ where: { id: tenantId } });
  if (!tenant) throw new ApiError(404, 'المستأجر غير موجود', 'TENANT_NOT_FOUND');
  ...
  const usageCount = await prisma.customer.count({ where: { outcomeId } });
  if (usageCount > 0) {
    throw new ApiError(
      409,
      `لا يمكن حذف النتيجة "${outcome.label}" — مرتبطة بـ ${usageCount} عميل. أزل Relationship أولاً.`,
      'OUTCOME_IN_USE',
    );
  }

  await prisma.tenantOutcome.delete({ where: { id: outcomeId } });
