...
export function AgentBuilder({ adminTenantId }: { adminTenantId?: string } = {}) {
  const { t } = useI18n();
  ...
  const isAdmin = Boolean(adminTenantId);

  useEffect(() => {
    if (isAdmin && adminTenantId) {
      api<Agent>(`/admin/tenants/${adminTenantId}/agent`)
  ...
  async function save() {
    ...
    try {
      if (isAdmin && adminTenantId) {
        await api<Agent>(`/admin/tenants/${adminTenantId}/agents/${agent.id}`, { method: 'PUT', json: agent });
      } else {
