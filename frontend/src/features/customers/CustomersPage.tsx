...
export function CustomersPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    const params: Record<string, string> = { page: String(page), pageSize: String(pageSizeValue) };
  ...
  }, [page, search, statusFilter, outcomeFilter, dateFrom, dateTo, pageSizeValue]);

  useEffect(() => { load(); }, [load]);

  useEffect(() => { setPage(1); setSelected(new Set()); setSelectAllMatching(false); }, [search, statusFilter, outcomeFilter, dateFrom, dateTo]);
  useEffect(() => { setPage(1); setSelected(new Set()); setSelectAllMatching(false); }, [pageSizeValue]);
  ...
  async function handleDelete() {
    ...
    try {
      const body: Record<string, unknown> = selectAllMatching
        ? { allMatching: true, filters: { search: search || undefined, status: statusFilter || undefined, outcomeId: outcomeFilter || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined } }
  ...
  async function handleCreateCampaign() {
    ...
    try {
      const body: Record<string, unknown> = selectAllMatching
        ? { name: campaignName.trim(), type: campaignType, message: campaignMessage || null, allMatching: true, filters: { search: search || undefined, status: statusFilter || undefined, outcomeId: outcomeFilter || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined } }
          ...
          <div className="flex items-center justify-between border-t border-[#E5E7EB] px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="h-8 rounded-lg border border-[#D0D5DD] bg-white px-2 text-[12px] text-[#111111]"
              >
