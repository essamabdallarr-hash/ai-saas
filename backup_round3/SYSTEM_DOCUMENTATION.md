...
تتبع **48 اختبارًا** باستخدام **vitest**، تُغطّي:
...
8. **QR Code في Dashboard** — بطاقة Dashboard تعرض نافذة منبثقة (Modal) تتحقق من حالة الاتصال وتعرض رمز QR مع polling كل 3 ثوانٍ. عند CONNECTED يتوقف الـ polling. لا يوجد redirect إلى صفحة WhatsAppSetup.
...
10. **TenantOutcome صلاحيات** — المستأجرون يقرأون نتائجهم فقط (`GET /outcomes`). الإنشاء/التعديل/الحذف عبر مسارات Admin فقط (`/admin/tenants/:id/outcomes`) مع `requireSuperAdmin`. لا يوجد安全感 حذف نتائج مرتبطة بعملاء (يُعاد 409 `OUTCOME_IN_USE`).
