...
export class RapidaProxyService {
  ...
  /** Active bidi streams keyed by assistantConversationId — used for sending disconnect through the existing stream */
  ...
  listenForCallEvents(rapidaConversationId: number, onEvent: (event: Record<string, unknown>) => void): () => void {
    ...
    this.activeStreams.set(rapidaConversationId, stream);
    ...
    stream.on('end', () => {
      this.activeStreams.delete(rapidaConversationId);
   ...
   * يرسل ConversationDisconnection (USER) عبر التدفق النشط إن وُجد،
   * وإلا يفتح تدفقًا جديدًا.
   */
  async disconnectConversation(rapidaConversationId: number): Promise<void> {
    if (!rapidaConversationId) return;

    const activeStream = this.activeStreams.get(rapidaConversationId);
    if (activeStream) {
      const nowSeconds = Math.floor(Date.now() / 1000);
      activeStream.write({
        disconnection: {
