import { Building2, KeyRound, ListChecks, Plus, Search, Sparkles, UserPlus, Users } from 'lucide-react';
...
import { OutcomesManager } from '@/features/customers/OutcomesManager';
...
export function TenantManager() {
  ...
  // إدارة النتائج
  const [outcomesOpenFor, setOutcomesOpenFor] = useState<string | null>(null);
  ...
  async function toggleUsers(t: TenantListItem) {
    ...
    outcomesOpenFor === t.id && setOutcomesOpenFor(null);
          ...
          <table className="w-full text-right text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {filtered.map((t) => (
                <Fragment key={t.id}>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => { setOutcomesOpenFor(outcomesOpenFor === t.id ? null : t.id); usersOpenFor === t.id && setUsersOpenFor(null); }}
                        className="flex items-center gap-1.5 rounded-lg border border-slate-600 px-3 py-1.5 text-xs text-slate-300 transition-colors hover:border-slate-500 hover:text-white"
                      >
                ...
                {outcomesOpenFor === t.id && (
                  <tr className="bg-slate-900/60">
                    <td colSpan={7} className="px-4 py-3">
                      <div className="max-w-md">
                        <OutcomesManager tenantId={t.id} />
                      </div>
                    </td>
                  </tr>
                )}
