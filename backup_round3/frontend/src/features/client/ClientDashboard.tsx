      ...
      <Modal open={qrOpen} onClose={() => setQrOpen(false)} title={t.clientDashboard.qrTitle}>
        {qrLoading ? (
          ...
          <div className="space-y-4">
            ...
            {(qrConn.status === 'BROKEN' || qrConn.status === 'BANNED') && (
              <div className="flex flex-col items-center gap-2">
                ...
                <Button variant="secondary" size="sm" onClick={() => { setQrOpen(false); window.location.href = '/workspace/whatsapp'; }}>
                  {t.clientDashboard.qrButton}
                </Button>
              </div>
            )}
          ...
          <div className="flex flex-col items-center gap-3">
            <p className="text-sm text-[#667085]">{t.clientDashboard.qrHint}</p>
            <Button variant="secondary" onClick={() => { setQrOpen(false); window.location.href = '/workspace/whatsapp'; }}>
              {t.clientDashboard.qrButton}
            </Button>
          </div>
        )}
