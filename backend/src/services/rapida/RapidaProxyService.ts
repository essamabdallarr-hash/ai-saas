  ...
  private callCreatePhoneCall(
    ...
    return new Promise((resolve, reject) => {
      ...
      client.createPhoneCall(request, (err: Error | null, response: any) => {
        if (err) return reject(new ApiError(502, `فشل بدء المكالمة عبر Rapida: ${err.message}`, 'RAPIDA_CALL_FAILED'));
        if (!response?.success) {
  ...
  async disconnectConversation(rapidaConversationId: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const stream = client.assistantTalk((err: Error | null) => {
        if (err) reject(new ApiError(502, `فشل إيقاف الوكيل عبر Rapida: ${err.message}`, 'RAPIDA_DISCONNECT_FAILED'));
        else resolve();
      });
